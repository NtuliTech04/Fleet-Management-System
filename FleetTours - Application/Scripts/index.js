...
function UploadToCloud() {
    document.getElementById('report').innerHTML = "Scanning In Progress...";
    context.drawImage(video, 0, 0, 640, 480);
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.d.length != 0) {
                ...
                window.location = "/CarHire/Success"; //If QRCode == "Delivery Confirmed"
        ...
        error: function (data) {
            document.getElementById('report').innerHTML = "Please focus camera on the QRCode";
            window.location = "/CarHire/Error"; //If QRCode != "Delivery Confirmed"
            $("#upload").removeAttr('disabled');
            $("#upload").attr("value", "Upload");
        },
        async: false
...
function stopCall() {
    document.getElementById('report').innerHTML = "Scanning Complete."
    clearTimeout(timer);
}
...
function take_snapshot() {
    ...
    webcam.set_api_url('/CarHire/UploadFlash?type=' + selection);
...
function stopScanning() {
    document.getElementById('upload_results').innerHTML = "Scanning Complete"
    clearTimeout(timer);
}
